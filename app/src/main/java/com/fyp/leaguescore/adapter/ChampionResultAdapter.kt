...
class ChampionResultAdapter (var champions:  MutableList<Champion>, var context: Context) :
    RecyclerView.Adapter<ChampionResultAdapter.ViewHolder>() {
    ...
    class ViewHolder(itemView: View, val context: Context) : RecyclerView.ViewHolder(itemView) {
        fun bind(champion: Champion) {
            ...
            if (!champion.avatar.isNullOrEmpty()){
                Glide.with(context).load(BuildConfig.API_URL+champion.avatar).error(R.drawable.avatar_placeholder).placeholder(
                    R.drawable.avatar_placeholder).into(itemView.imgch);
            }
